...
def test_the_block_never_carries_content(identity, config, monkeypatch, tmp_path):
    ...
    for one of them.
    """
    # Plant at the paths the app RESOLVES, not just any temp dir: repoint
    # APPDATA so rekounts.paths lands here, then write through those helpers.
    # A future collector that reads via rekounts.paths — the natural mistake —
    # now finds these plants and fails the secret scan below.
    monkeypatch.setenv("APPDATA", str(tmp_path))
    from rekounts import paths
    paths.app_data_dir().mkdir(parents=True, exist_ok=True)
    paths.scratchpad_path().write_text(SCRATCHPAD, encoding="utf-8")
    paths.history_path().write_text(TRANSCRIPT, encoding="utf-8")
