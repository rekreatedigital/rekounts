...
| `text_inserter.py` | `TextInserter` — paste insertion behind a Win32/macOS backend; native clipboard save/restore across all formats, modifier-release wait, elevation (UIPI) and focus-change guards; returns an `InsertResult` outcome. A keystroke path still exists but is **config-file-only** (`insertion_mode` + `long_text_via_paste`, neither in Settings) — synthesized VK_PACKET events corrupt text in WinUI/XAML apps at any length, so it is reserved for apps that ignore Ctrl+V |
